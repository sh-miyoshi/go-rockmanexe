...
func (e *enemyAquaman) Process() (bool, error) {
	...
	case aquamanActTypeShot:
		...
		if e.count == 0 {
			e.actID = localanim.AnimNew(skill.Get(skillcore.SkillAquamanShot, skillcore.Argument{
	...
	case aquamanActTypeBomb:
		if e.count == 3*aquamanDelays[aquamanActTypeBomb] {
			// ボム登録
			localanim.AnimNew(skill.Get(skillcore.SkillWaterBomb, skillcore.Argument{
				OwnerID:    e.pm.ObjectID,
				Power:      50,
				TargetType: damage.TargetPlayer,
			}))
